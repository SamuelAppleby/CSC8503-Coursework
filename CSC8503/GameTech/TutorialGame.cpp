...
void TutorialGame::InitCamera() {
	...
	world->GetMainCamera()->SetPosition(Vector3(0, 20, -50));
	lockedObject = nullptr;
}
...
void TutorialGame::InitMixedGridWorld(int numRows, int numCols, float rowSpacing, float colSpacing) {
	...
	for (int x = 0; x < numCols; ++x) {
		for (int z = 0; z < numRows; ++z) {
			...
			if (type == 1) {
				if(rand() % 2)
					AddCubeToWorld(position, cubeDims * Vector3(1, 2, 1));
				else
					AddCubeToWorld(position, cubeDims);
			}
			else if (type == 2) {
				if (rand() % 2)
					AddSphereToWorld(position, sphereRadius / 2);
				else
					AddSphereToWorld(position, sphereRadius);
			}
			else {
				if (rand() % 2)
					AddCapsuleToWorld(position, 3.0f, sphereRadius * 1.5);
				else
	...
	if (inSelectionMode) {
		...
		if (Window::GetMouse()->ButtonDown(NCL::MouseButtons::LEFT)) {
			...
			if (world->Raycast(ray, closestCollision, true)) {
				...
				ray.SetDirection(worldPos + fwd * 2);
				RayCollision closestCollision;
				if ((world->Raycast(ray, closestCollision, true))) {
					GameObject* hitObj = (GameObject*)closestCollision.node;
					hitObj->GetRenderObject()->SetColour(Vector4(0, 0, 1, 1));
				}
				return true;
			}
			else {
